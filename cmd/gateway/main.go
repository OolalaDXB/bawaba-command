...
func main() {
	...
	// P3 Souffleur: translates real decision records, never decides. No
	// provider configured → the endpoint answers 503, never a canned text.
	if prov, err := souffleur.FromEnv(); err != nil {
		logger.Error("souffleur provider misconfigured — Souffleur disabled", "error", err)
	} else if prov != nil {
		apiSrv.AttachSouffleur(prov)
		logger.Info("souffleur enabled", "provider", prov.Name(), "model", prov.Model())
	} else {
		logger.Info("souffleur not configured (BAWABA_SOUFFLEUR_PROVIDER unset) — explain endpoint will answer 503")
	}
